        ...
        try:
            ...
            if platform_name == 'windows':
                ...
                script_lines.extend([
                    ...
                    'set "PROCESS_CHECK_ATTEMPTS=0"',
                    ':WAIT_FOR_PROCESS_EXIT',
                    'set "PROCESS_RUNNING="',
                    'for /f "usebackq tokens=1 delims=," %%P in (`tasklist /FI "IMAGENAME eq %EXECUTABLE_NAME%" /FO CSV /NH`) do (',
                    '    if /I "%%~P"=="%EXECUTABLE_NAME%" set "PROCESS_RUNNING=1"',
                    ')',
                    ...
                    ')',
                    'set "PROCESS_RUNNING_ALT="',
                    'for /f "usebackq tokens=1 delims=," %%P in (`tasklist /FI "IMAGENAME eq NaverBlogAutomation.exe" /FO CSV /NH`) do (',
                    '    if /I "%%~P"=="NaverBlogAutomation.exe" set "PROCESS_RUNNING_ALT=1"',
                    ')',
                    ...
                    'goto PROCESS_READY',
                    ':MANUAL_CLOSE_PROMPT',
                    'echo [WARNING] 프로그램이 여전히 실행 중입니다. 직접 종료한 뒤 아무 키나 누르세요.',
                    'pause',
                    'set "PROCESS_CHECK_ATTEMPTS=0"',
                    'goto WAIT_FOR_PROCESS_EXIT',
                    ':PROCESS_READY',
                    'echo [INFO] 실행 중인 프로세스가 모두 종료되었습니다.',
