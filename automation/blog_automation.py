...
class BlogAutomation:
    ...
    def setup_driver(self):
        ...
        try:
            ...
            self.post_interaction = PostInteraction(self.driver, self.logger, self.buddy_manager)
    ...
    def search_and_collect_blogs(self, keyword, target_count, start_page=1):
        ...
        while len(collected_blogs) < target_count and current_page < start_page + max_attempts:
            self.logger.info(f"📄 페이지 {current_page}에서 수집 중... (현재 {len(collected_blogs)}/{target_count}개)")
            ...
            # 현재 페이지에서 블로그 수집
            raw_blogs = self.blog_searcher.collect_blog_names(page_target, current_page)
            ...
            # 블로그 아이디 추출
            blog_ids = [blog.get('blog_name', '') for blog in raw_blogs if blog.get('blog_name')]
            ...
            if excluded_count > 0:
                self.logger.info(f"페이지 {current_page}에서 이미 추출된 아이디 {excluded_count}개 제외")
            ...
            self.logger.info(f"페이지 {current_page}에서 {len(new_blogs)}개 새 블로그 추가 (총 {len(collected_blogs)}/{target_count}개)")
            
            # 목표 달성 확인
            if len(collected_blogs) >= target_count:
                self.logger.info(f"🎯 목표 달성! {len(collected_blogs)}개 새로운 블로그 수집 완료")
                break
                
            current_page += 1
        
        if len(collected_blogs) < target_count:
            self.logger.warning(f"⚠️ 목표 미달성: {len(collected_blogs)}/{target_count}개만 수집됨 (최대 {max_attempts}페이지 탐색)")
    ...
    def collect_neighbor_blogs(self, blog_url, target_count=None):
        ...
        if target_count is None:
            ...
            if target_count is None:
                self.logger.error("❌ 이웃 수집 개수가 설정되지 않았습니다. GUI에서 이웃 개수를 설정해주세요.")
        ...
        # 이웃커넥트 가용성 확인
        is_available, error_message = self.neighbor_collector.check_neighbor_connect_availability(blog_id)
        if not is_available:
        ...
        while len(collected_new_ids) < target_count and current_page <= max_pages:
            self.logger.info(f"📄 이웃커넥트 페이지 {current_page} 수집 중... (현재 {len(collected_new_ids)}/{target_count}개 새로운 아이디)")
            ...
            # 이미 추출된 아이디 제외
            new_ids_from_page = self.extracted_ids_manager.filter_new_ids(page_blog_ids)
            ...
            self.logger.info(f"✅ 페이지 {current_page}: 총 {len(page_blog_ids)}개, 이미 추출된 아이디 {excluded_count}개 제외, 새로운 아이디 {len(truly_new_ids)}개 추가")
            self.logger.info(f"🏃 현재 수집된 새로운 아이디: {len(collected_new_ids)}/{target_count}개")
            
            # 목표 달성 확인
            if len(collected_new_ids) >= target_count:
                self.logger.info(f"🎆 목표 달성! {len(collected_new_ids)}개 새로운 아이디 수집 완료")
                break
        ...
        try:
            ...
            for i, data in enumerate(blog_data, 1):
                ...
                if not self.buddy_manager.buddy_available:
                    self.logger.warning(f"❌ {blog_name} 모바일 서로이웃 추가 불가능 - 건너뛰기")
                    continue
                ...
                if not enable_like and not enable_comment:
                    self.logger.info(f"🚫 [{blog_name}] 공감/댓글 모두 비활성화 - 서이추만 완료하고 다음 블로그로 이동")
                    continue
                
                # 공감/댓글 중 하나라도 활성화된 경우 게시글로 이동
                # 서로이웃 추가 후 메인 블로그 페이지에서 최신 게시글로 이동 (모바일 방식)
                latest_post_success = self.buddy_manager.navigate_to_latest_post_mobile(blog_name)
                if not latest_post_success:
                    self.logger.warning(f"❌ {blog_name} 최신 게시글 이동 실패 - 건너뛰기")
                    continue

                # 현재 페이지에서 게시글 상호작용 처리 (모바일 방식)
                interaction_success = self.post_interaction.process_current_page_interaction(blog_name)
            ...
            self.logger.info(f"이웃커넥트 모바일 블로그 처리 완료: {success_count}/{total_count} 성공")
    ...
    def process_keyword_blog_automation(self, blog_data, progress_callback=None):
        ...
        try:
            ...
            for i, data in enumerate(blog_data, 1):
                ...
                self.logger.info(f"[{i}/{total_count}] {blog_name} 키워드 블로그 처리 중...")
            ...
            success_count = self.buddy_manager.get_buddy_success_count()
            self.logger.info(f"키워드 블로그 처리 완료: {success_count}/{total_count} 성공")
            return success_count, total_count
